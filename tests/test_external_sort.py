...
from external_sort import external_multithreaded_natural_merge_sort as my_sort
...
class TestExternalSortOneFile(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.file_name = f"{self.dir_name}/test_sort_one_file.txt"
...
class TestExternalSortCSVFile(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """Создание файлов перед тестом."""
        self.dir_name = TEST_FOLDER
        self.file_name = f"{self.dir_name}/test_sort_csv.csv"
        if not os.path.exists(self.dir_name):
            os.mkdir(self.dir_name)
        if not os.path.exists(self.file_name):
            open(self.file_name, "x", encoding="utf-8").close()
...
class TestExternalSortTwoFile(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """Создание файлов перед тестом."""
        self.dir_name = TEST_FOLDER
        self.file_name_first = f"{self.dir_name}/test_sort_more_txt_files_1.txt"
        self.file_name_second = f"{self.dir_name}/test_sort_more_txt_files_2.txt"
        if not os.path.exists(self.dir_name):
            os.mkdir(self.dir_name)
    ...
    def test_sort_more_files_with_output(self) -> None:
        ...
        output = f"{TEST_FOLDER}/test_sort_more_txt_files_output.txt"
